...
$sql_transaksi = sql("SELECT * FROM transaksi WHERE id_user='$_SESSION[id_pelanggan]' AND `status`='Sedang Diproses' ORDER BY id_transaksi DESC");
include "header.php";
?>
  ...
  <div class="row border-bottom pt-4 d-flex justify-content-center align-items-center">
    ...
    if ($sql_transaksi->num_rows > 0) {
      foreach ($sql_transaksi as $transaksi) { ?>
        <div class="row d-flex justify-content-center align-items-center" style="width: 100%;">
          ...
          <div class="col-2">
            <a href="terima_pesanan.php?id=<?= $transaksi['id_transaksi']; ?>" class="btn btn-success" onclick="return confirm('Pesanan sudah diterima?')">Terima Pesanan</a>
          </div>
        </div>
        <?php
    ...
    <?php } else { ?>
      <div class="row border-top border-bottom main d-flex justify-content-center align-items-center mx-4">
        <div class=" alert alert-info mt-5 mb-5 text-center" role="alert">
